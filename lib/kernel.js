...
Kernel.prototype.makePeriod = function(cases) {
    if (cases == 0) {
        ...
        for (var index=0; index<=(diff/(hour*24) + 1); index++) {
            var cronDate = new Date(this.query_data.created_date).getTime() + (24 * index * hour) + 1000 * 60 * 10;
            if (cronDate > end_date) {
                cronDate = end_date + 1000 * 60 * 10;
            }
        ...
        var crons = [];
        for (var index=0; index<=(diff/(hour*24) + 1); index++) {
            var cronDate = new Date(this.query_data.start_date).getTime() + (24 * index * hour) + 1000 * 60 * 10;
            if (cronDate > end_date) {
                cronDate = end_date + 1000 * 60 * 10;
