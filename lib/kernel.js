...
Kernel.prototype.buildSchedule = function(self) {
    ...
    if (this.query_data.start_date < this.query_data.created_date && this.query_data.end_date < this.query_data.created_date) {
        ...
        for (var index=0; index<this.period_data.crons.length; index++) {
            new CronJob(this.period_data[index], function() {
        ...
        for (var index=1; index<this.period_data.crons.length; index++) {
    ...
    async.waterfall([
        ...
        ResultModel.load(self.result_id, function(err, resultObj) {
            ...
            if (resultObj) {
                ...
                var until = new Date(self.query_data.end_date).setHours(0, 0, 0, 0);
...
Kernel.prototype.makePeriod = function(cases) {
    if (cases == 0) {
        ...
        var crons = [];
        console.log(diff/(hour*24));
        for (var index=0; index<=(diff/(hour*24) + 1); index++) {
            var cronDate = new Date(this.query_data.created_date).setHours(new Date(this.query_data.created_date).getHours() + (24 * index) + 1);
        ...
        for (var index=0; index<=(diff/(hour*24)); index++) {
            var cronDate = new Date(this.query_data.start_date).setHours(new Date(this.query_data.start_date).getHours() + (24 * index) + 1);
            if (cronDate > end_date) {
                cronDate = end_date;
            }
            var cronPattern = utils.makeCronPattern(cronDate);
