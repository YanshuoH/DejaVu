...
Kernel.prototype.makePeriod = function(cases) {
    if (cases == 0) {
        ...
        for (var index=0; index<=(diff/(hour*24)); index++) {
            var cronDate = new Date(this.query_data.created_date).setHours(new Date(this.query_data.created_date).getHours() + (24 * index)) + 1000 * 60 * 3;
            if (cronDate > end_date) {
                cronDate = end_date + 1000 * 60 * 3;
            }
        ...
        var crons = [];
        for (var index=0; index<=(diff/(hour*24)); index++) {
            var cronDate = new Date(this.query_data.start_date).setHours(new Date(this.query_data.start_date).getHours() + (24 * index)) + 1000 * 60 * 3;
            if (cronDate > end_date) {
                cronDate = end_date + 1000 * 60 * 3;
            }
            var cronPattern = utils.makeCronPattern(cronDate);
