...
exports.run = function(query_data, cb) {
    async.waterfall([
        ...
        function(callback) {
            var options = {
                criteria: {"created_at" : {"$gt": new Date(query_data.start_date).toISOString(), "$lt": new Date(query_data.end_date).toISOString()}}
            };
            TweetModel.list(options, function(err, list) {
                callback(null, list);
            });
        },
        // for each tweet, find an array of tweets satisfy conditions
