    ...
    twitter.search(params, auth_data.accessToken, auth_data.accessTokenSecret, function(err, data, response) {
        ...
        if (data.statuses.length == 0) {
                ...
                async.waterfall([
                    function(callback) {
                        for (var index=0; index<data.statuses.length; index++) {
                            ...
                            // console.log(data.statuses[index].user);
                            if (utils.fetchEvents(self.query_data.events, data.statuses[index].text)
                                && (data.statuses[index].place || data.statuses[index].coordinates)) {
            ...
            if (typeof(data.search_metadata.next_results) !== 'undefined') {
                ...
                console.log('===========Twitter Search: no more results============');
            }
        }
    });
}
...
TwitterSearch.prototype.retweetFilter = function(text, query_events) {
