...
OmnipotentCollector.prototype.connect = function() {
    ...
    this.UserModel = con.model('UserModel', UserModelFile.UserModelSchema);
    this.TweetModel = con.model('TweetModel', TweetModelFile.TweetModelSchema);
    this.StreamingModel = con.model('StreamingModel', StreamingModelFile.StreamingModelSchema);
    // =========FIN============
}
...
OmnipotentCollector.prototype.getStatus = function(cb) {
    this.connect();
    this.StreamingModel.loadByStatus(1, cb);
}

OmnipotentCollector.prototype.getDBInfo = function(cb) {
    async.waterfall([
        function(callback) {
            this.UserModel.count({}, function(err, user_count) {
                callback(null, user_count);
            });
        },
        function(user_count, callback) {
            this.TweetModel.count({}, function(err, tweet_count) {
