...
OmnipotentCollector.prototype.getDBInfo = function(self, cb) {
    async.waterfall([
        function(callback) {
            self.UserModel.count({}, function(err, user_count) {
                callback(null, user_count);
            });
        },
        function(user_count, callback) {
            self.TweetModel.count({}, function(err, tweet_count) {
...
OmnipotentCollector.prototype.exportJson = function(self, cb) {
    async.waterfall([
        ...
        function(msg, callback) {
            self.UserModel.listToJson({}, function(err, users) {
                var user = {};
                user.users = users;
                var filename = './public/tmp/users.json';
                console.log('Writing to: ' + filename);
                fs.writeFile(filename, JSON.stringify(users, null, 4), function(err) {
                    if(err) {
                      console.log(err);
                    } else {
        ...
        function(user_filename, callback) {
            self.TweetModel.listToJson({}, function(err, tweets) {
                var tweet = {};
                tweet.tweets = tweets;
