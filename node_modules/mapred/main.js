...
var mapreduce_simple = function(pieces, map, reduce, callback){
    ...
    for (var index=0; index<pieces.length; index++) {
        var key = pieces[index][0];
        var value = pieces[index][1];
    ...
    for (var index=0; index<intermediate.length; index++) {
        var position = utils.checkUserExisting(intermediate[index][0], user_list);
        if (position == -1) {
            groups.push({
                users: intermediate[index][0],
                tweets: [intermediate[index][1]]
            });
            user_list.push(intermediate[index][0]);
        }
        else {
            // User pair exists
            // check if tweets pair exiting
            if (utils.checkTweetExisting(intermediate[index][1], groups[position].tweets) > -1) {
                groups[position].tweets.push([
